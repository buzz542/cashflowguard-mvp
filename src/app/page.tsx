  ...
  const ProfileButton = () => {
    ...
    return (
      <div className="relative">
        <button onClick={(e) => { e.stopPropagation(); setShowProfile(!showProfile); }} className="w-9 h-9 rounded-full bg-blue-600 text-white text-sm font-semibold flex items-center justify-center">{user.email.charAt(0).toUpperCase()}</button>
        {showProfile && (
          <div className="absolute right-0 mt-2 w-56 bg-white border rounded-xl shadow-lg p-3 z-30" onClick={(e) => e.stopPropagation()}>
            <p className="text-xs text-gray-500">Signed in as</p>
            <p className="text-sm font-medium truncate">{user.email}</p>
            <p className="text-xs text-gray-500 mt-1">{user.isPro ? "Pro plan · unlimited checks" : user.freeUsed ? "Free check used" : "Free check available"}</p>
            {!user.isPro && <button onClick={() => { setShowProfile(false); setShowSubscribe(true); }} className="mt-2 w-full text-left text-sm text-blue-600 px-2 py-1.5">Upgrade to Pro</button>}
            <button onClick={logout} className="mt-1 w-full text-left text-sm text-red-600 px-2 py-1.5">Log out</button>
          </div>
        )}
      </div>
    );
      ...
      <div className="min-h-screen flex items-center justify-center px-4 bg-[#FAFAF9]">
        <div className="bg-white rounded-2xl border p-6 w-full max-w-sm space-y-4">
          ...
          <p className="text-xs text-center text-gray-500">
            {authMode === "signup" ? <>Have an account? <button className="text-blue-600" onClick={() => setAuthMode("login")}>Log in</button></> : <>New? <button className="text-blue-600" onClick={() => setAuthMode("signup")}>Sign up</button></>}
          </p>
          <button className="text-xs text-gray-400 w-full" onClick={() => setAuthMode(null)}>Cancel</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4 bg-black/40">
        <div className="bg-white rounded-2xl border p-6 w-full max-w-sm space-y-4">
          ...
          {authError && <p className="text-sm text-red-600">{authError}</p>}
          <button className="w-full bg-blue-600 text-white font-semibold py-3 rounded-xl" disabled={checkoutLoading} onClick={startCheckout}>{checkoutLoading ? "Opening Stripe…" : "Subscribe with Stripe"}</button>
          <button className="w-full text-sm text-gray-500" onClick={() => setShowSubscribe(false)}>Maybe later</button>
        </div>
      </div>
    );
  }
          ...
          <>
                ...
                {reviews.length > 0 && (
                  <div className="space-y-3">
                    ...
                    {reviews.slice(0, 3).map((r) => (
                      <button key={r.id} onClick={() => { setResult(r.result); setStep("results"); }} className="w-full text-left bg-white rounded-2xl border p-4">
                        <div className="flex justify-between text-sm"><span className="font-medium">{r.trade || "Check"}</span><span className="text-gray-500">{r.date}</span></div>
                      </button>
                    ))}
                  </div>
                )}
            ...
            {step === "context" && (
              <div className="space-y-6">
                ...
                <form onSubmit={handleContextSubmit} className="bg-white rounded-2xl border p-5 space-y-4">
                  <div><label className="block text-sm font-medium mb-1">Trade / work</label><input required type="text" className="w-full rounded-lg border px-3 py-2.5" value={context.trade} onChange={(e) => setContext({ ...context, trade: e.target.value })} /></div>
                  <div><label className="block text-sm font-medium mb-1">Package size</label>
                  ...
                  <div><label className="block text-sm font-medium mb-1">Your role</label>
            ...
            {step === "results" && (
              <div className="space-y-6">
                ...
                <button onClick={startCheck} className="w-full bg-blue-600 text-white font-semibold py-3.5 rounded-xl">Start another check →</button>
              </div>
            )}
          </>
        )}
          ...
          <div className="space-y-4">
            ...
            {reviews.map((r) => (
              <div key={r.id} className="bg-white rounded-2xl border p-5">
                <div className="flex justify-between text-sm"><span className="font-medium">{r.trade}</span><span className="text-gray-500">{r.date}</span></div>
                <details className="text-sm mt-2"><summary className="cursor-pointer text-blue-600">View result</summary><div className="mt-3" dangerouslySetInnerHTML={{ __html: formatResult(r.result) }} /></details>
              </div>
            ))}
          </div>
        )}
        {tab === "about" && (
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl border p-5 text-sm text-gray-700">
              ...
              {user?.isPro && <p className="text-blue-700 font-medium mt-2">You are on the Pro plan.</p>}
            </div>
          </div>
        )}
      </main>
