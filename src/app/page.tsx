...
export default function HomePage() {
      ...
      <div className="min-h-screen flex items-center justify-center px-4 bg-[#F5F4F0]">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 w-full max-w-sm space-y-4">
          ...
          <form onSubmit={handleAuth} className="space-y-3">
            ...
            <input type="password" required placeholder="Password (min 6)" className="w-full rounded-lg border border-gray-300 px-3 py-2.5" value={password} onChange={(e) => setPassword(e.target.value)} />
            {authError && <p className="text-sm text-red-600">{authError}</p>}
            <button type="submit" className="w-full bg-[#111] text-white font-semibold py-3 rounded-xl hover:bg-black">{authMode === "signup" ? "Create account" : "Log in"}</button>
          </form>
          <p className="text-xs text-center text-gray-500">
            {authMode === "signup" ? (
              <>
                Have an account? <button type="button" className="text-blue-600 font-medium" onClick={() => setAuthMode("login")}>Log in</button>
              </>
            ) : (
              <>
                New? <button type="button" className="text-blue-600 font-medium" onClick={() => setAuthMode("signup")}>Sign up</button>
              </>
            )}
          </p>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4 bg-black/40">
        <div className="bg-white rounded-2xl border p-6 w-full max-w-sm space-y-4">
          ...
          {authError && <p className="text-sm text-red-600">{authError}</p>}
          <button className="w-full bg-[#111] text-white font-semibold py-3 rounded-xl" disabled={checkoutLoading} onClick={startCheckout}>
  ...
  /* ─── MARKETING HOMEPAGE (Luminance-style structure) ─── */
  if (view === "marketing") {
    return (
      <div className="min-h-screen flex flex-col bg-[#F5F4F0]">
        {/* Nav */}
        <header className="sticky top-0 z-30 bg-[#F5F4F0]/95 backdrop-blur border-b border-gray-200/80">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <img src="/logo.svg" alt="GuardConstruct" className="h-8 w-auto" />
              <span className="font-bold text-base tracking-tight hidden sm:inline">GuardConstruct</span>
            </div>
            ...
            <div className="flex items-center gap-3">
              {user ? (
                <button type="button" className="text-sm text-gray-600 hover:text-gray-900" onClick={() => { setUser(null); localStorage.removeItem("gc_user"); }}>Log out</button>
              ) : (
                <button type="button" onClick={() => setAuthMode("login")} className="text-sm text-gray-600 hover:text-gray-900">Log in</button>
              )}
              <button type="button" onClick={startCheck} className="bg-[#F5E642] text-gray-900 text-sm font-semibold px-4 py-2 rounded-lg hover:bg-[#efe22e]">
                Check a document
              </button>
            </div>
          </div>
        ...
        <section className="max-w-4xl mx-auto px-4 sm:px-6 pt-16 pb-12 text-center">
          <p className="text-xs font-semibold tracking-[0.2em] text-gray-500 uppercase mb-5">Cash flow protection</p>
          ...
          <button type="button" onClick={startCheck} className="mt-8 bg-[#F5E642] text-gray-900 font-semibold px-8 py-3.5 rounded-xl text-base hover:bg-[#efe22e]">
        ...
        {/* Trust bar */}
        <section className="border-y border-gray-200/80 bg-white/50 py-8">
        ...
        {/* Features */}
        <section id="features" className="bg-white border-y border-gray-200/80 py-16">
        ...
        <section className="max-w-5xl mx-auto px-4 sm:px-6 py-16">
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6 sm:col-span-2 sm:max-w-lg sm:mx-auto">
              <p className="text-amber-400 text-sm mb-3">★★★★★</p>
        ...
        {/* Pricing */}
        <section id="pricing" className="bg-white border-y border-gray-200/80 py-16">
          <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
            ...
            <div className="grid sm:grid-cols-2 gap-6 text-left">
              ...
              <div className="rounded-2xl border-2 border-gray-900 p-6">
                ...
                <button type="button" onClick={startCheck} className="w-full bg-[#111] text-white font-semibold py-2.5 rounded-xl hover:bg-black">Get Pro</button>
              </div>
            </div>
          </div>
        </section>

        {/* Roadmap — competitor-inspired, honest */}
        ...
        {/* Final CTA */}
        <section className="border-t border-gray-200 bg-white py-14 text-center px-4">
          ...
          <button type="button" onClick={startCheck} className="mt-6 bg-[#F5E642] text-gray-900 font-semibold px-8 py-3.5 rounded-xl hover:bg-[#efe22e]">
            Check a document
          </button>
        </section>

        <footer className="border-t border-gray-200 py-8 text-center text-xs text-gray-500 px-4">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
          <button type="button" onClick={() => { setView("marketing"); setStep("landing"); }} className="flex items-center gap-2">
            ...
            <span className="font-bold text-lg hidden sm:inline">GuardConstruct</span>
          </button>
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-6 pb-24">
        {step === "landing" && (
          <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-3">
            ...
            <button type="button" onClick={startCheck} className="w-full bg-[#111] text-white font-semibold py-3.5 rounded-xl">Start a new check →</button>
          </div>
        )}
        {step === "context" && (
          <form onSubmit={(e) => { e.preventDefault(); setStep("upload"); }} className="bg-white rounded-2xl border border-gray-200 p-5 space-y-4">
            <h1 className="text-2xl font-bold">About this job</h1>
            <input required placeholder="Trade / work" className="w-full rounded-lg border border-gray-300 px-3 py-2.5" value={context.trade} onChange={(e) => setContext({ ...context, trade: e.target.value })} />
            ...
            <button type="submit" className="w-full bg-[#111] text-white font-semibold py-3.5 rounded-xl">Continue →</button>
          </form>
        )}
        {step === "upload" && (
          <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-4">
            ...
            {/* Separate inputs: camera vs documents */}
            <input
              ref={photoRef}
              type="file"
              accept="image/*"
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className="border-2 border-dashed border-gray-300 rounded-xl py-5 text-sm font-medium disabled:opacity-60 hover:border-gray-400"
              >
            ...
            {pages.length > 0 && (
              <ul className="text-sm space-y-1">
                {pages.map((p, i) => (
                  <li key={i} className="bg-gray-50 rounded-lg px-3 py-2 border border-gray-100">{i + 1}. {p}</li>
                ))}
              </ul>
            )}
            ...
            <button
              ...
              className="w-full bg-[#111] text-white font-semibold py-3.5 rounded-xl disabled:opacity-50"
            >
              Run check →
            </button>
          </div>
        ...
        {step === "loading" && (
          <div className="text-center py-16">
            ...
            <h1 className="text-xl font-bold mt-4">Checking the document…</h1>
            <p className="text-sm text-gray-500 mt-2">Looking for payment traps under English law</p>
          </div>
        )}
        {step === "results" && (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <button type="button" className="text-sm text-blue-600 font-medium" onClick={() => setStep("landing")}>Done</button>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-5 text-sm whitespace-pre-wrap leading-relaxed">{result}</div>
            <button type="button" onClick={startCheck} className="w-full bg-[#111] text-white font-semibold py-3.5 rounded-xl">Start another check →</button>
          </div>
        )}
      </main>
      <footer className="border-t border-gray-200 py-4 text-center text-xs text-gray-500">Commercial risk identification only. Not legal advice.</footer>
    </div>
  );
}
